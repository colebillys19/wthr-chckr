import { v4 as uuidv4 } from "uuid";

type SvgComponentPropsType = {
  size?: number;
};

function SvgComponent({ size = 240 }: SvgComponentPropsType) {
  const uniqueIdA = uuidv4();
  const uniqueIdB = uuidv4();
  const uniqueIdC = uuidv4();
  const uniqueIdD = uuidv4();

  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 240 240"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        d="M152.973 186.258H62.941C54.6921 186.258 46.524 184.633 38.9031 181.476C31.2821 178.32 24.3576 173.693 18.5248 167.86C12.692 162.027 8.06511 155.103 4.90841 147.482C1.75172 139.861 0.126984 131.693 0.126984 123.444C0.126984 115.195 1.75172 107.027 4.90841 99.4059C8.06511 91.785 12.692 84.8604 18.5248 79.0276C24.3576 73.1948 31.2821 68.568 38.9031 65.4113C46.524 62.2546 54.6921 60.6298 62.941 60.6298C66.7646 60.6283 70.5805 60.9743 74.3416 61.6635C80.7076 48.3534 90.3392 36.8712 102.339 28.287C117.171 17.6736 134.679 12.0635 152.973 12.0635C173.07 12.0675 192.548 19.0206 208.105 31.7443C223.662 44.4681 234.341 62.1797 238.332 81.8771C242.323 101.574 239.38 122.046 230.002 139.821C220.624 157.597 205.389 171.583 186.877 179.41C176.152 183.95 164.62 186.28 152.973 186.258Z"
        fill={`url(#${uniqueIdA})`}
      />
      <path
        d="M50.7302 227.955C43.802 227.955 38.9931 221.054 41.3893 214.555L47.9086 196.874C48.1216 196.297 48.5062 195.799 49.0107 195.448C49.5152 195.096 50.1153 194.908 50.7302 194.908C51.3449 194.908 51.9447 195.097 52.449 195.448C52.9532 195.8 53.3377 196.297 53.5505 196.874L60.0699 214.555C62.4661 221.054 57.6572 227.955 50.7302 227.955Z"
        fill={`url(#${uniqueIdB})`}
      />
      <path
        d="M120.098 227.955C113.171 227.955 108.361 221.054 110.758 214.555L117.277 196.874C117.49 196.297 117.874 195.8 118.379 195.448C118.883 195.097 119.483 194.908 120.098 194.908C120.712 194.908 121.312 195.097 121.817 195.448C122.321 195.8 122.705 196.297 122.918 196.874L129.437 214.555C131.835 221.054 127.025 227.955 120.098 227.955Z"
        fill={`url(#${uniqueIdC})`}
      />
      <path
        d="M189.465 227.955C182.538 227.955 177.729 221.054 180.126 214.555L186.645 196.874C186.858 196.297 187.242 195.8 187.747 195.448C188.251 195.097 188.851 194.908 189.465 194.908C190.08 194.908 190.68 195.096 191.185 195.448C191.689 195.799 192.074 196.297 192.287 196.874L198.806 214.555C201.202 221.054 196.394 227.955 189.465 227.955Z"
        fill={`url(#${uniqueIdD})`}
      />
      <defs>
        <linearGradient
          id={uniqueIdA}
          x1="0.125715"
          y1="99.1606"
          x2="240.07"
          y2="99.1606"
          gradientUnits="userSpaceOnUse"
        >
          <stop stopColor="#69BBFF" />
          <stop offset="1" stopColor="#9DE0F4" />
        </linearGradient>
        <linearGradient
          id={uniqueIdB}
          x1="127.472"
          y1="119.694"
          x2="-3.21771"
          y2="279.344"
          gradientUnits="userSpaceOnUse"
        >
          <stop stopColor="#69BBFF" />
          <stop offset="1" stopColor="#1798FF" />
        </linearGradient>
        <linearGradient
          id={uniqueIdC}
          x1="169.007"
          y1="153.696"
          x2="38.3162"
          y2="313.345"
          gradientUnits="userSpaceOnUse"
        >
          <stop stopColor="#69BBFF" />
          <stop offset="1" stopColor="#1798FF" />
        </linearGradient>
        <linearGradient
          id={uniqueIdD}
          x1="210.542"
          y1="187.696"
          x2="79.8513"
          y2="347.345"
          gradientUnits="userSpaceOnUse"
        >
          <stop stopColor="#69BBFF" />
          <stop offset="1" stopColor="#1798FF" />
        </linearGradient>
      </defs>
    </svg>
  );
}

export default SvgComponent;
